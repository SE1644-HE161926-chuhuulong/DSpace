...
const GroupDetails = () => {
    ...
    const handleDeleteGroup = async () => {
        try {
            const response = await axios.delete(`https://localhost:7200/api/Group/deleteGroup/${groupId}`);
            if (response.status === 200) {
                setSuccessMessage('Group deleted successfully');
                setFailureMessage('');
                navigate('/Dspace/Group/ListOfGroup');
            } else {
    ...
    return (
        <div>
            ...
            <div className="container mt-5">
                ...
                <div className="mt-4">
                    <button className="btn btn-success me-2" onClick={() => navigate(`/Dspace/Group/addUsertoGroup/${group.groupId}`)}>Add people to group</button>
                    <button className="btn btn-primary me-2" onClick={() => navigate(`/Dspace/Group/editGroup/${group.groupId}`)}>Edit</button>
                    <button className="btn btn-danger me-2" onClick={handleDeleteGroup}>Delete</button>
                    <button className="btn btn-secondary" onClick={() => navigate('/Dspace/Group/ListOfGroup')}>Back to List</button>
                </div>
            </div>
        </div>
    );
