...
{
    public static IServiceCollection AddDatabaseModule<T>(this IServiceCollection services,
        IConfiguration configuration) where T : DbContext
    {
        services.AddDbContext<T>(context =>
        {
            context.UseSqlServer(configuration.GetConnectionString("DSpaceDbContext"));
        });
        services.AddScoped<DbContext>(
            provider => provider.GetService<T>() ?? throw new ArgumentNullException(nameof(T)));
        return services;
    }
